...
test("current paid test has a fixed Xiaohongshu asset output folder", async () => {
  ...
  const longImagePrompt = await readFile(new URL("long-image-prompt.md", assetRoot), "utf8");
  ...
  assert.match(readme, /long-image\.png/);
  assert.match(readme, /long-copy\.md/);
  assert.match(coverPrompt, /小红书原生测试封面风/);
  assert.match(coverPrompt, /不要真人写实孤独照片/);
  assert.match(longImagePrompt, /长文引流配图/);
  assert.match(longImagePrompt, /不要给出完整测试答案/);
...
test("family origin paid test has Xiaohongshu cover and long-copy sources", async () => {
  ...
  const longImagePrompt = await readFile(new URL("long-image-prompt.md", familyAssetRoot), "utf8");
