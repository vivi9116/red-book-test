...
const preferenceRawStyle = `3:4竖版，小红书手帐拼贴风，温暖米色旧纸背景，真实纸纹，关系观察笔记，便签拼贴，浅色手帐题卡，胶带，回形针，干花，旧照片，爱心，红笔箭头，铅笔涂鸦。
...
function preferenceBrand() {
  return [
    paper(92, 110, 510, 112, { fill: "#fffaf2", stroke: "#dec1a3", opacity: 0.78, rotate: -3, dash: "10 9" }),
    text("懂你的X学姐", 165, 186, 48, { weight: 600, rotate: -3 }),
    text("❤", 505, 165, 38, { color: "#cf5968", weight: 700, rotate: -10 }),
  ].join("");
}
...
function preferenceCoverOverlay(page) {
  ...
  ${svgFilters()}
  ${preferenceBrand()}
  ${text(page.title[0], 980, 345, 90, { anchor: "middle", weight: 800, rotate: -2 })}
  ${text(page.title[1], 980, 490, 102, { anchor: "middle", color: "#b23a4e", weight: 800, rotate: -2 })}
  ${text(page.title[2], 980, 638, 102, { anchor: "middle", color: "#b23a4e", weight: 800, rotate: -2 })}
  ${brush(665, 720, 390, 86, "#f5d783", 0.62, -2)}
  ${text(page.badge, 860, 782, 60, { anchor: "middle", weight: 800, rotate: -2 })}
  ${brush(265, 1970, 1150, 125, "#f8e6a5", 0.66, -1)}
  ${text(page.footer, 840, 2052, 56, { anchor: "middle", color: "#a43b4b", weight: 800, rotate: -1 })}
`;
}
...
function preferenceQuestionCard(question, spec, index) {
  const parsed = splitQuestion(question);
  const accent = spec.accent ?? ["#df6570", "#efa93a", "#c85a66"][index % 3];
  const badgeFill = spec.badgeFill ?? accent;
  const titleY = spec.y + 96;
  const optionStartY = spec.y + 175;
  const lineGap = spec.lineGap ?? 62;
  const body = `
    <path d="${tornPath(spec.x, spec.y, spec.w, spec.h)}" fill="${spec.fill ?? "#fff8ed"}" fill-opacity="${spec.opacity ?? 0.83}" stroke="${spec.stroke ?? "#e2bd8b"}" stroke-width="4" stroke-dasharray="13 10" filter="url(#paperShadow)"/>
    <circle cx="${spec.x + 78}" cy="${spec.y + 82}" r="54" fill="${badgeFill}" fill-opacity="0.92" stroke="#fffaf2" stroke-width="8"/>
    ${text(`${parsed.number}.`, spec.x + 78, spec.y + 101, 58, { anchor: "middle", color: "#fffaf2", weight: 800 })}
    ${brush(spec.x + 200, spec.y + 58, spec.w - 310, 62, accent, 0.2)}
    ${text(parsed.title, spec.x + 225, titleY, spec.titleSize ?? 44, { weight: 800 })}
    ${parsed.options.map(([label, value], optionIndex) => {
      const y = optionStartY + optionIndex * lineGap;
      return `
        <rect x="${spec.x + 230}" y="${y - 38}" width="74" height="48" rx="12" fill="${accent}" fill-opacity="0.9"/>
        ${text(`${label}.`, spec.x + 267, y, 35, { anchor: "middle", color: "#fffaf2", weight: 800 })}
        ${text(value, spec.x + 335, y, spec.optionSize ?? 36, { weight: 600 })}
      `;
    }).join("")}
  `;
  return rotateGroup(spec.x, spec.y, spec.w, spec.h, spec.rotate ?? 0, body);
}

function preferenceTitle(page, y = 250, rotate = -1) {
  return `
    ${paper(210, y - 88, 1260, 130, { fill: "#c99a67", stroke: "#b98556", opacity: 0.78, rotate, dash: "0" })}
    ${text(page.title, 840, y, 67, { anchor: "middle", color: "#a83f55", weight: 800, rotate })}
  `;
}

function preferenceFooter(value, y = 2070, fill = "#f8e6a5") {
  return `
    ${brush(260, y - 72, 1160, 118, fill, 0.72, -1)}
    ${text(value, 840, y, 54, { anchor: "middle", color: "#a43b4b", weight: 800, rotate: -1 })}
  `;
}
...
function preferenceQuestionsOverlay(page) {
  const variants = {
    "preference-questions": [
      { x: 120, y: 430, w: 1435, h: 390, rotate: -0.8, accent: "#df6570" },
      { x: 150, y: 900, w: 1395, h: 390, rotate: 0.7, accent: "#eca23b" },
      { x: 115, y: 1370, w: 1435, h: 410, rotate: -0.5, accent: "#d85e70" },
    ],
    "preference-questions-alt": [
      { x: 210, y: 430, w: 1265, h: 365, rotate: 1.2, accent: "#d75d67", fill: "#fff2df" },
      { x: 125, y: 880, w: 1320, h: 375, rotate: -1.4, accent: "#a8834a", fill: "#fff8ed" },
      { x: 230, y: 1340, w: 1250, h: 390, rotate: 0.9, accent: "#d65f65", fill: "#fbf1df" },
    ],
    "preference-questions-ticket": [
      { x: 120, y: 430, w: 1360, h: 360, rotate: -1.2, accent: "#ba4657", fill: "#fff7e8" },
      { x: 210, y: 900, w: 1275, h: 365, rotate: 1.1, accent: "#d3983d", fill: "#fff4df" },
      { x: 135, y: 1370, w: 1375, h: 395, rotate: -0.7, accent: "#b94d58", fill: "#fff8ef" },
    ],
  };
  ...
  return `
    ...
    ${page.questions.map((question, index) => preferenceQuestionCard(question, specs[index], index)).join("")}
    ${preferenceFooter(page.footer)}
  `;
}

function preferenceResultCard(line, x, y, width, color, rotate) {
  return rotateGroup(x, y, width, 120, rotate, `
    <path d="${tornPath(x, y, width, 120)}" fill="#fffaf2" fill-opacity="0.82" stroke="#ddc3a0" stroke-width="4" filter="url(#paperShadow)"/>
    ${text(line, x + 52, y + 76, 48, { color, weight: 800 })}
  `);
}

function preferenceResultOverlay(page) {
  const questionSpec = { x: 150, y: 520, w: 1385, h: 360, rotate: -0.8, accent: "#ba4657", fill: "#fff7e8" };
  return `
    ${svgFilters()}
    ${preferenceBrand()}
    ${preferenceTitle(page)}
    ${preferenceQuestionCard(page.questions[0], questionSpec, 0)}
    ${preferenceResultCard(page.results[0], 270, 1040, 1040, "#a43b4b", -1.2)}
    ${preferenceResultCard(page.results[1], 345, 1215, 1010, "#a43b4b", 1)}
    ${preferenceResultCard(page.results[2], 275, 1390, 1070, "#a43b4b", -0.6)}
    ${preferenceFooter(page.footer[0], 1965, "#d7877d")}
    ${text(page.footer[1], 840, 2045, 45, { anchor: "middle", color: "#a43b4b", weight: 800, rotate: -1 })}
  `;
}
