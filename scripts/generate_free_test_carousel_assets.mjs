...
const DEFAULT_TEST_ID = "active-vs-withdrawn-like";
...
const ACTIVE_FOLDER = "web/assets/free-tests/active-vs-withdrawn-like";
...
const preferencePages = [
  ...
  },
  {
    ...
    footer: "选最符合真实想法的一项。",
    questions: [
      ["4. 收到礼物时，你更在意？", ["A. 对方有没有花心思（3分）", "B. 礼物是否适合你当下需要（2分）", "C. 对方是否真的观察过你（1分）"]],
...
底部撕纸写：选最符合真实想法的一项。
重点核对这些字，不要写错：花心思、朋友夸你们般配、真的聊得来。
装饰放边缘，不能压住文字。不要乱码，不要错别字，不要多余文字。`,
  },
  {
    file: "04-q7-9.png",
...
const activeRawStyle = `3:4竖版，小红书可发布的可爱手帐测试日记风，不要像网页截图、PPT、Word文档、程序画图、代码生成模板。
...
const activeVsWithdrawnNativePages = [
  {
    file: "01-cover.png",
    prompt: `${activeRawStyle}
直接生成完整封面。
...
const TESTS = {
  ...
  "preference-vs-understanding-like": {
    ...
    directPages: preferenceNativePages,
    rawPages: [],
    pages: [],
  },
  "active-vs-withdrawn-like": {
    folder: ACTIVE_FOLDER,
    directPages: activeVsWithdrawnNativePages,
    rawPages: [],
    pages: [],
  },
};
...
function preferenceQuestionsOverlay(page) {
  const variants = {
    ...
    "preference-questions-alt": [
      { x: 390, y: 555, rotate: -0.4 },
      { x: 390, y: 940, rotate: 0.3 },
      { x: 390, y: 1325, rotate: -0.2 },
    ],
